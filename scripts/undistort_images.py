...
fisheye_output_dir = os.path.join(CURRENT_PATH, '../data/undistorted_images/fisheye')
pinhole_output_dir = os.path.join(CURRENT_PATH, '../data/undistorted_images/pinhole')
os.makedirs(fisheye_output_dir, exist_ok=True)
os.makedirs(pinhole_output_dir, exist_ok=True)

for image_name, image in instaCamFisheye.raw_images():
    instaCamFisheye.undistort_image(
        image,
        image_name,
        ...
        show_image=False,
        save_image=True,
        output_path=os.path.join(fisheye_output_dir, image_name),
    )
    
for image_name, image in instaCamPinhole.raw_images():
    instaCamPinhole.undistort_image(
        image,
        image_name,
        calibration_filename='pinhole_calibration.json',
        balance=1,
        show_image=False,
        save_image=True,
        output_path=os.path.join(pinhole_output_dir, image_name),
    )
